...
{
    uint32 talentPoints = getLevel() < 10 ? 0 : getLevel() - 9;
    if (getClass() == CLASS_DEATH_KNIGHT)
        talentPoints = getLevel() < 56 ? 0 : getLevel() - 55;
    talentPoints += m_questRewardTalentCount;
    talentPoints += m_extraBonusTalentCount;

    return uint32(talentPoints * sWorld->getRate(RATE_TALENT));
}
